...
class SignatureVerifier:
    ...
    def get_public_value(self) -> Any:
        self.assert_contract_is_instantiated()
        return self.contract.getPublicValue.call()
    ...
    def set_public_value(self, new_value: int, transaction_config) -> Any:
        self.assert_contract_is_instantiated()
        return self.contract.setPublicValue(new_value, transaction_config)
...
def handle_get_public_value(args: argparse.Namespace) -> None:
    network.connect(args.network)
    contract = SignatureVerifier(args.address)
    result = contract.get_public_value()
    print(result)
...
def handle_set_public_value(args: argparse.Namespace) -> None:
    network.connect(args.network)
    contract = SignatureVerifier(args.address)
    transaction_config = get_transaction_config(args)
    result = contract.set_public_value(
...
def generate_cli() -> argparse.ArgumentParser:
    ...
    get_public_value_parser = subcommands.add_parser("get-public-value")
    add_default_arguments(get_public_value_parser, False)
    get_public_value_parser.set_defaults(func=handle_get_public_value)
    ...
    set_public_value_parser = subcommands.add_parser("set-public-value")
    add_default_arguments(set_public_value_parser, True)
    set_public_value_parser.add_argument(
        "--new-value", required=True, help="Type: uint256", type=int
    )
    set_public_value_parser.set_defaults(func=handle_set_public_value)
